class Avatar_video:
    ...
    def __init__(self, path_videos=None):
        ...
        self.black_screen = self.crear_fondo_poner_videos()

    def crear_fondo_poner_videos(self):
    ...
    def calcular_nueva_dimension(self, actual, marco=0.8):
        ...
        if self.sistema_iniciado:
            if self.avatar_inicio:
                ...
                if self.hay_video_mostrar:
                    ...
                    if self.hay_video_mostrar:
                        dimen = self.calcular_nueva_dimension(self.frame.shape, 0.7)
                    ...
                    if self.hay_avatar:
                        ...
                        self.avatar = self.cv2.VideoCapture(self.PATH_VIDEOS + "/" + self.AVATAR['BAS1']) if self.multiplexor_avatar else self.cv2.VideoCapture(self.PATH_VIDEOS + "/" + self.AVATAR['BAS2'])
        else:
    ...
    def continuar_siguiente_paso_instruccion(self):
        self.path_video_actual = self.LISTADO_VIDEOS_INSTRUCCIONES[self.instruccion_actual]['path']
    ...
    def set_instruccion_actual(self, value):
        self.instruccion_actual = value
